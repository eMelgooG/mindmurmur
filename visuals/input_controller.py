...
class InputController(object):
    ...
    def bind_keyboardevents(self, guiframe):
        ...
        self.guiframe.Bind(wx.EVT_MOUSEWHEEL, self.OnWheel)

    def unbind_keyboardevents(self):
        self.guiframe.UnBind(wx.EVT_KEY_DOWN)
        self.guiframe.UnBind(wx.EVT_MOUSEWHEEL)
        self.guiframe = None
    ...
    def OnKeyDown(self, event):
        ...
        # 1 or [F1] - STATE 1
        if key_code == 49 or key_code == 340:
            self.engine.set_meditation_state(1)
            event.Skip()
        # 2 - STATE 2
        if key_code == 50:
            self.engine.set_meditation_state(2)
            event.Skip()
        # 3 - STATE 3
        if key_code == 51:
            self.engine.set_meditation_state(3)
            event.Skip()
        # 4 - STATE 4
        if key_code == 52:
            self.engine.set_meditation_state(4)
            event.Skip()
        # 5 - STATE 5
        if key_code == 53:
            self.engine.set_meditation_state(5)
            event.Skip()
        # [F2] - STATE minus
        if key_code == 341:
            self.engine.set_meditation_state(set_prev = True)
            event.Skip()
        # [F3] - STATE plus
        if key_code == 342:
            self.engine.set_meditation_state(set_next = True)
            event.Skip()
        # [F8] - START
        if key_code == 347:
            self.engine.start()
            event.Skip()
        # ESC OR [F9] OR [F10] - STOP
        elif key_code == 27 or key_code == 348 or key_code == 349:
            self.engine.stop()
            event.Skip()
